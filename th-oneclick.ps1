...
function Upsert([string]$k,[string]$v){
  $p=".\\.env.local"; if(-not(Test-Path $p)){New-Item -ItemType File -Path $p|Out-Null}
  $t=Get-Content $p -Raw
  if($t -match "(?m)^$k="){ $t=($t -replace "(?m)^$k=.*","$k=$v") } else { $t+="`r`n$k=$v" }
  $t|Set-Content $p -Encoding UTF8
}
function Wait-Health([string]$o,[int]$s=90){
  for($i=0;$i -lt [Math]::Ceiling($s/2);$i++){
    try{$r=Invoke-RestMethod "$o/api/health" -TimeoutSec 2; if($r.ok){return $true}}catch{}
    Start-Sleep 2
  }; return $false
}

# vyber port a srovnej ENV (nešahá na tajné KEY/BASE)
$Port = ($Ports | Where-Object { -not (Get-NetTCPConnection -LocalPort $_ -State Listen -ErrorAction SilentlyContinue) } | Select-Object -First 1)
...
$dev=(Get-Command pnpm -ErrorAction SilentlyContinue) ? "pnpm dev -- -p $Port -H $BindHost" : "npm run dev -- -p $Port -- -H $BindHost"
...
try{
  ...
  $preview = ($chat.content ?? "") -replace '\r?\n',' '
